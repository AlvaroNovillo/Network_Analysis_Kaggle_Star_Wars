...
```{r}
plot(starwars_op,giant_component,main="Karate network with maximal modularity")
```
...
plot(starwars_fg,giant_component,main="Karate network with fast greedy")
```
...
plot(starwars_lv,giant_component,main="Karate network with Louvain")
```
...
plot(starwars_lp,giant_component,main="Karate network with label propagation")
```
...
plot(starwars_wa,giant_component,main="Karate network with walktrap")
```
...
plot(starwars_eb,giant_component,main="Karate network with edge betweenness")
```
...
We are going to exclude optimal modularity due to its high computational cost. We generate 10000 networks using both the Erdös and Rényi model with 110 vertices and 444 edges, and a generalized random graph using the degree sequence:
...
In conclusion, although in the first part we used our knowledge of the topic to select subnetworks that were of interest, community detection using classical and generalized random graphs do not give any robust result about the numbe rof communities and their components.
